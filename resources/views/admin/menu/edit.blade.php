            ...
            @else
                <form method="POST" action="{{ url('/admin/menus/' . $menu->id) }}" class="form-horizontal">
                            ...
                            <select class="form-control" id="category_id" name="category_id" required>
                                ...
                                @foreach ($categories as $item)
                                    <option value="{{ $item->id }}" 
                                        @if(old('category_id', $menu->category_id) == $item->id) selected @endif>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            @php
                                // Tentukan nilai yang dicentang (menggunakan old() atau data dari DB)
                                $hasLevel = old('has_level', $menu->has_level);
                            @endphp
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="has_level" id="level_ya" value="1" {{ $hasLevel == 1 ? 'checked' : '' }} required>
                                <label class="form-check-label" for="level_ya">Ya (Contoh: Mie Pedas)</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="has_level" id="level_tidak" value="0" {{ $hasLevel == 0 ? 'checked' : '' }} required>
